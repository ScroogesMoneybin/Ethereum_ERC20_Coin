...
describe("StableCoin", function () {
  let ethUsdPrice: number, feeRatePercent: number;
  ...
  this.beforeEach(async () => {
    ...
    let originalSupply = await StableCoin.totalSupply();
    console.log("Orinal Amount",originalSupply);
    
  });

  it("Should set fee rate percentage", async function () {
    expect(await StableCoin.feeRatePercent()).to.equal(feeRatePercent);
    // const x=await StableCoin.feeRatePercent();
    // console.log("fee rate percentage:",x);
  });

  it("Should allow minting", async function () {
    const ethAmount = 1;
    const expectedMintAmount = ethAmount * ethUsdPrice;
    let amounts = ethers.utils.parseEther(ethAmount.toString());
    await StableCoin.mint({
      value: amounts,
    });
    let TotalSupply = await StableCoin.totalSupply();
    let AmountMint = expectedMintAmount.toString();
    
    console.log("Amounts:", amounts);
    console.log("total supply:", TotalSupply);
    console.log("ExpectedMintAmount:", AmountMint);
    
    expect(TotalSupply).to.equal(ethers.utils.parseEther(AmountMint));
  ...
  describe("With minted tokens", function () {
    ...
    it("Should allow burning", async function () {
      ...
      const difference= mintAmount - remainingStableCoinAmount;
    ...
    it("Should prevent depositing collateral buffer below minimum", async function () {
      ...
      await expect(
        ...
        `custom error 'InitialDpcCollateralRatioError("STC: Initial collateral ratio not met. Minimum amount is:", ` +
